...
const loadBookData = () => {
    ...
    //when input  data matched
    else {
        const url = `https://openlibrary.org/search.json?q=${searchText}`;
        fetch(url)
            // console.log(url)
            .then(res => res.json())
            .then(data => displayBookData(data))//call displayBookData function
    }
...
const displayBookData = data => {
    ...
    if (booksInfo.length === 0) {
       ...
       <h1 class="bg-danger w-75 mx-auto rounded-3 text-center p-2">Sorry, No search results found!!</h1>
